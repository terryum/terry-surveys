...
`survey_harness/` owns the runtime-neutral task DAG, quality profiles, score history, remediation attempts, resume state, and release evidence. Codex and Claude are dispatch adapters: one orchestrator runs at most three workers, using KG mapper → source strategy → foundations/frontier research → critical analysis/editorial contract → evidence synthesis → representative chapter review → paired KO/EN chapter writing → streamed image/factcheck → chapter QA → book QA. Role contracts live in `.codex/skills/survey/references/role-contracts-v2.md`; mutable thresholds live only in `survey_harness/config/quality_profiles.yaml`.

Canonical Codex skills live in `.codex/skills/`; project discovery at
`.agents/skills/{survey,tutorial}` must link to those sources. Use the survey
`scripts/sync_installed.py --apply` installer, then `--check`; it preserves old
discovery material outside the active path. `--archive-legacy` explicitly
archives only the three superseded project orchestrators. Audit other legacy
skills and native agent definitions before changing them. Do not invoke stale
team APIs or use book-specific native agent prompts for a different topic.
Task packets and their fingerprints, scoped file ownership, and real execution
IDs are the handoff contract. Prose length and table volume are editorial
signals; factual evidence and independent passage-level review remain required.
