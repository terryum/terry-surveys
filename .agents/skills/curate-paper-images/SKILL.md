...
4. **OpenAI gpt-image-2 생성 개념도**: 현재 런타임의 `image_gen` 도구와 `$imagegen` 스킬 — 이론·전략·생태계 챕터의 overview 스키마, 타임라인, 비교 다이어그램. 기본은 `--style survey-dark --ratio 16:9 --quality medium`; Terry가 결과물을 마음에 들지 않는다고 재생성을 지시할 때만 `--quality high` 또는 `--high`를 쓴다. **티어 쿼터 내에서 다수 생성 허용** (이전의 "챕터당 ≤ 2" 상한 폐기).
...
- 캡션이 길면 1-2문장으로 요약
- 생성 일러스트는 개념도임을 밝히고 실제 생성 도구·모델 provenance를 기록한다
...
이론/전략/생태계 챕터는 논문 figure로 커버되지 않는 시각 자료가 챕터 본문을 이해하는 데 핵심이다. OpenAI gpt-image-2로 다음 유형을 **적극** 생성한다. 신규 생성은 현재 런타임의 `image_gen` 도구와 `$imagegen` 스킬을 사용한다. 지원되지 않는 CLI 경로나 인자를 만들지 않는다. 실제 도구가 반환한 모델·생성 정보를 provenance에 기록한다:
