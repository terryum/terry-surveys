...
- cwd가 `/Users/terrytaewoongum/Codes/personal/terry-surveys` 내부이고
  `bash scripts/setup-contents.sh --check`가 통과.
- Python 3, `python3 build.py --help`가 응답.
- Codex skill의 `references/agent-template/` 디렉토리와 그 하위 6개 템플릿 md + README 존재. deep-researcher 템플릿은 bootstrap 시 foundations/frontier 2개 에이전트로 확장된다.
- 루트 `bibtex/references.bib` 존재 (인덱스 재생성에 필요).
- `source-repositories.md`의 GitHub source gate 통과.
  `terryum/terry-surveys-contents`가 `PRIVATE`여야 한다.
...
### Step 2. 비공개 콘텐츠 구조 스캐폴딩

```bash
python3 build.py --new <slug>
```

`shared/scaffold.py`가 `surveys` 심링크를 통해
`terry-surveys-contents/surveys/<slug>`에 canonical 구조를 만든다:
...
python3 ~/.codex/skills/survey/scripts/sync_agents.py <slug> --retrofit --apply \
  --repo-root /Users/terrytaewoongum/Codes/personal/terry-surveys \
  --skill-dir ~/.codex/skills/survey
```

**또는** `scripts/bootstrap.sh <slug> <title_ko> <title_en> <domain> [--visibility=group --group=<grp>] [--repo-root=<path>] [--skill-dir=<path>]`으로 위 전 과정 한 번에 실행.
...
```bash
cd ../terry-surveys-contents
git add surveys/<slug>/ bibtex/refs_index.json
git commit -m "feat(<slug>): bootstrap survey scaffold + agents"
```

**사용자가 명시 동의할 때만 커밋.** 기본은 변경사항만 만들고 커밋은 사용자 재량.
커밋 전 `source-repositories.md`에 따라 contents remote가 private인지 다시
확인한다. reader-public 서베이도 public framework repo에 source를 push하지 않는다.
...
DONE. next steps:
  ...
  3) Run: /survey --orchestrate <slug>
     - v2 controller가 ready task만 bounded dispatch하고 실제 agent ID와 산출물을 기록
