...
- **에이전트 역할 = 공통, 도메인 = 고유**: v2 controller의 KG → source strategy → parallel research → critical analysis/editorial contract → evidence synthesis → bilingual writing → image/factcheck → independent QA 흐름은 모든 책이 공유한다.
- **한 곳에서 발전**: 팀이 집필 중 발견한 개선 사항(새 에러 케이스, 새 팀 통신 패턴)을 이 템플릿에 반영하면, 다음 새 책은 자동으로 최신 버전을 복사해간다.
- **기존 책 sync는 선택**: `/survey --sync-agents <slug>` 호출 시에만 기존 책의 `.claude/agents/`와 diff를 검토한 뒤 사용자가 요청한 범위 내에서 동기화.
...
| `critical-analyst.md` | 비교·상충 근거·해결/미해결 문제 |
| `book-writer.md` | 책별 편집 계약·양국어 챕터 병행 집필 |
...
Codex reads the canonical templates through controller task packets. Runtime communication is optional; files, declared ownership, and actual execution receipts are the handoff contract. Legacy Claude files remain compatibility artifacts, not instructions to call unavailable APIs.
