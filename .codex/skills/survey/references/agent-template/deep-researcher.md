...
Full survey에서 본인 샤드는 다음 최소치를 만족해야 완료다.

```
profile = survey_harness/config/quality_profiles.yaml
target_corpus = max(profile.corpus_floor_base, profile.corpus_floor_per_chapter * chapter_count)
# Shard proportions are topic-dependent and declared in search_protocol.md.
```

- 두 shard 모두 비어 있으면 안 되며, `search_protocol.md`의 시대·주제 coverage와
  두 번 연속 신규 eligible source 증가율 5% 미만인 포화 조건을 만족해야 한다.
- 7장 이상 survey는 full survey로 본다. mini/smoke 예외는 사용자가 명시한
  경우에만 허용된다.
- 최소 편수 미달, rich metadata 90% 미달, 또는 원문 검증 플래그 누락은
  "완료"가 아니라 `BLOCKED: research shard below floor`다.
- 빠르게 끝났다는 것은 실패 신호일 수 있다. shallow starter list를 제출하지
  말고 coverage expansion pass를 수행한다.
...
```bash
python3 ~/.codex/skills/survey/scripts/merge_research_shards.py {{SURVEY_SLUG}} \
...
- **송신**:
  ...
  - `evidence-librarian` (dedup·gap·chapter packet), `book-writer` (집필 자료 공급, 특히 seminal 알림), `fact-checker` (인용 키 source of truth)
- **수신**:
...
- [ ] full survey 편수 hard gate를 만족했는가 (`foundations_min` 또는 `frontier_min`)
- [ ] `papers_{{RESEARCHER_ROLE}}.json`에 각 엔트리의 `method_summary`가 3문장 이상, 숫자 포함
- [ ] rich metadata 비율 90%+ 인가 (`method_summary`, `limitations`, `experiments` 또는 `quantitative_results`, `tags`, `chapter_hint`)
