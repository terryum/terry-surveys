...
- **chapter-level trail**: `_factcheck_report.md`는 summary-only가 아니라 각 챕터의
  inspected refs, corrected refs, unresolved refs, load-bearing claims, and
  verification command results를 남긴다. 12장 full survey에서 5-10줄짜리
  factcheck report는 release evidence가 아니다.
- **과도한 표준화 금지**: 저자 이름 표기(Kim vs 김)는 해당 논문의 공식 영문 표기를 따른다. 추측 금지.
- **재검증 루프**: 한 번 "ok" 처리한 ref라도 해당 논문의 v2·v3가 arXiv에 올라오면 재검증.
- **release-grade coverage**: full survey release는 load-bearing claims 100%
  verified, 전체 `_refs_extracted.json` verification coverage 90%+를 목표로
  한다. 90% 미만이면 source-type ceiling과 남은 위험을 `_factcheck_report.md`
  에 명시하고 QA는 release를 막아야 한다.
...
5. **중복 author-year**: 같은 챕터 reference에 동일 `Author (Year)`가 둘 이상 있으면 동일 항목은 병합하고, 서로 다른 항목이면 `(2025a)/(2025b)` suffix를 부여한다. `build_site.py`의 citation map이 한 citation을 여러 reference로 해석하게 두지 않는다.
6. **citation UX 전제**: 빌드 후 HTML에 `class="cite-link"`, 안정적 `sup id`, reference `li id`, 그리고 최신 citation으로 갱신되는 `.cite-backlink`가 존재하는지 확인한다.

수정 후 재검증: `unresolved citation` 0건, 산문형 author-year citation 0건, duplicate author-year ambiguity 0건이 reference 정합성의 1차 게이트. fact-checker는 이 게이트를 통과시킨 다음에 arxiv/DOI 검증으로 넘어간다.
...
- [ ] verification coverage ≥ 90% 또는 source-type ceiling이 명시됨
- [ ] `_factcheck_report.md`의 Summary 통계가 실 수정 건수와 일치
- [ ] `_factcheck_report.md`가 각 chNN을 명시하고, 어떤 claim/ref를 확인했는지 chapter-level audit trail을 남겼는가
