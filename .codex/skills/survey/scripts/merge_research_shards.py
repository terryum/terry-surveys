...
Usage:
  python3 merge_research_shards.py <survey-slug> [--repo-root /path/to/terry-surveys] [--check] [--strict] [--strict]
...
DEFAULT_MONOREPO = Path("/Users/terrytaewoongum/Codes/personal/terry-surveys")
MONOREPO = DEFAULT_MONOREPO


def detect_repo_root(explicit=None):
    if explicit:
        root = Path(explicit).expanduser().resolve()
        if (root / "build.py").exists() and (root / "surveys").is_dir():
            return root
        raise SystemExit(f"ERROR: --repo-root is not terry-surveys: {root}")
    if DEFAULT_MONOREPO.exists():
        return DEFAULT_MONOREPO
    here = Path(__file__).resolve()
    for parent in here.parents:
        if (parent / "build.py").exists() and (parent / "surveys").is_dir():
            return parent
    raise SystemExit("ERROR: cannot find terry-surveys repo root; pass --repo-root")
...
def main(argv=None):
    ...
    parser.add_argument("slug", help="survey slug, e.g. claude-to-codex")
    parser.add_argument("--repo-root", help="path to terry-surveys repo")
    parser.add_argument("--strict", action="store_true", help="accepted for compatibility; missing shard markdown is fatal in --check mode")
    parser.add_argument("--check", action="store_true",
                        help="verify shard discovery and JSON validity without writing merged outputs")
    args = parser.parse_args(argv)

    MONOREPO = detect_repo_root(args.repo_root)
    slug = args.slug
    ...
    if args.check:
        ...
        missing_timeline = []
        for name in shard_names:
            if not (research_dir / f"groups_{name}.md").exists():
                missing_groups.append(f"groups_{name}.md")
            if not (research_dir / f"timeline_{name}.md").exists():
                missing_timeline.append(f"timeline_{name}.md")
        print(f"CHECK OK: {len(shard_names)} shard(s), {dedup_stats['shard_entries']} entries, "
              f"{dedup_stats['unique_entries']} unique after dedup")
        if missing_groups or missing_timeline:
            print("ERROR: missing markdown shard artifact(s): "
                  + ", ".join(missing_groups + missing_timeline), file=sys.stderr)
            sys.exit(1)
        return
